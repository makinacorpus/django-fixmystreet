...
from django_fixmystreet.models import Ward, Report, ReportUpdate, ReportCategoryClass,ReportCategory,ReportSubscriber,DictToPoint,UserProfile
...
class ReportUpdateForm(forms.ModelForm):
    ...
    def __init__(self,data=None, files=None, initial={},
                 first_update=False,user = None, report=None):
       self.user = None
       self.report = report
       ...
       super(ReportUpdateForm,self).__init__(
           data, files=files, initial=initial)
       ...
       if first_update:
           ...
           f = forms.fields.BooleanField(
               required=False,
               help_text=_('This problem has been fixed.'),
               label='')
           self.fields['is_fixed'] = f

    def save(self,commit=True):
       update = super(ReportUpdateForm,self).save(
           commit = False)
       if self.report:
...
class ReportForm(forms.ModelForm):
    ...
    lat = forms.fields.CharField(
        widget=forms.widgets.HiddenInput)
    lon = forms.fields.CharField(
        widget=forms.widgets.HiddenInput)
    ...
    def __init__(self,
        ...
        self.pnt = d2p.pnt()
        self.ward = d2p.ward()
        self.update_form = ReportUpdateForm(
            data=data, initial=initial,
            user=user, first_update = True)

        super(
            ReportForm,self).__init__(
                data, files,
                initial=initial)
...
class FMSNewRegistrationForm(RegistrationForm):
    ...
    def save(self, profile_callback=None, site=None):
        if not site:
        ...
        user_profile.phone = self.cleaned_data.get(
            'phone', '')
        user_profile.save()
    ...
    def clean_email(self):
        if User.objects.filter(
            ...
            raise forms.ValidationError(
                _(u'That email is already in use.'))
        return self.cleaned_data['email']

    def send_email(self,new_user):
        registration_profile = (
            RegistrationProfile.objects.get(user=new_user))
        ...
        url = "%s/accounts/activate/%s/"
        ss =  settings.ACCOUNT_ACTIVATION_DAYS
...
class FMSAuthenticationForm(AuthenticationForm):
    """
    Base class for authenticating users. Extend this to
    get a form that accepts
